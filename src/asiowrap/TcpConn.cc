...
{
	socket_.async_read_some(boost::asio::buffer(recv_buffer_.WriteBegin(), recv_buffer_.ValidSize()),
...
void TCPConn::AsyncWrite()
{
	...
	async_writing_ = true;
	socket_.async_write_some(boost::asio::buffer(writing_buffer_.ReadBegin(), writing_buffer_.Size()),
		std::bind(&TCPConn::HandleWrite, shared_from_this(), std::placeholders::_1, std::placeholders::_2));
}

void TCPConn::HandleRead(boost::system::error_code err, std::size_t trans_bytes)
{
	if (err) {
...
void TCPConn::HandleWrite(boost::system::error_code err, std::size_t trans_bytes)
{
	async_writing_ = false;
